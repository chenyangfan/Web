...
<body>
  ...
  $my_query = "SELECT * FROM Posts WHERE UserID = 1 AND PostID = $postId";
  $result = selectQuery($conn, $my_query);
  $post = array();
  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-8 cold-md-10 mx-auto">
        ...
        $my_query = "SELECT * FROM Comments WHERE UserID = 1 AND PostID = $postId ORDER BY Date DESC";
        $result = selectQuery($conn, $my_query);
        if ($result->num_rows > 0) {
          while ($row = $result->fetch_assoc()) {
            $comment = array();
